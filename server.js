...
app.post("/api/inventory", verifyToken, verifyAdmin, async (req, res) => {
  try {
    console.log('Received inventory POST request with body:', req.body);
    const { 
      itemName, 
      itemType, 
      category, 
...
app.get("/api/dashboard/stats", verifyToken, verifyAdmin, async (req, res) => {
  try {
    ...
    // Fetch total products from inventory (active items only)
    const totalProducts = await InventoryItem.countDocuments({ isActive: true });

    // For POS system, total customers = total orders (each order represents a customer transaction)
  ...
  try {
    ...
    if (!referer || !referer.includes('/admindashboard/addstaff')) {
      return res.status(403).send(`
        <script>
          alert("Access denied. Use admin dashboard to register staff.");
    ...
    res.status(201).send(`
      <script>
        ...
        window.location.href = "/admindashboard/addstaff"; // Redirect back to form
      </script>
    `);
  } catch (err) {
    res.status(500).send(`
...
app.get("/admindashboard/dashboard", verifyToken, verifyAdmin, async (req, res) => {
  try {
    const totalOrders = await Order.countDocuments();
    // Changed: Count total products from inventory instead of Product model
    const totalProducts = await InventoryItem.countDocuments({ isActive: true });
